...
% 
% FORCESPRONLPsolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCESPRONLPsolver FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCESPRONLPsolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCESPRONLPsolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCESPRONLPsolver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCESPRONLPsolver_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCESPRONLPsolver_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCESPRONLPsolver_block';
    end
    ...
    function_name = 'FORCESPRONLPsolver_coderFunction.m';
    position = [170, 99, 650, 231];
    parameter_sizes = struct('x0', '[9,1]',...
                             'xinit', '[2,1]',...
                             'all_parameters', '[9,1]');
    if useCompactBlock
        function_name = 'FORCESPRONLPsolver_coderFunctioncompact.m';
        position = [170, 99, 650, 231];
        parameter_sizes = struct('x0', '[9,1]',...
                                 'xinit', '[2,1]',...
                                 'all_parameters', '[9,1]');
