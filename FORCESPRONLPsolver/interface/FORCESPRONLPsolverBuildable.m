...
% 
classdef FORCESPRONLPsolverBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'FORCESPRONLPsolver'))
                solverpath = fullfile(buildablepath, 'FORCESPRONLPsolver');
            end
            solverInfo = struct();
            solverInfo.solvername = 'FORCESPRONLPsolver';
        ...
        function [output, exitflag, info] = forcesInitOutputsMatlab()
            ...
            outputs_u0 = coder.nullcopy(zeros(1, 1));
            output = struct('u0', outputs_u0);
            
            exitflag = coder.nullcopy(0);
        end

        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = FORCESPRONLPsolverBuildable.forcesCall(params.x0, params.xinit, params.all_parameters);
        end

        function [output, exitflag, info] = forcesCall(x0, xinit, all_parameters)
            solvername = 'FORCESPRONLPsolver';
            ...
            [output_c, exitflag_c, info_c] = FORCESPRONLPsolverBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = FORCESPRONLPsolverBuildable.forcesInitOutputsMatlab();
            ...
            output.u0 = cast(output_c.u0, 'like', output.u0);
    ...
    methods (Static, Access = private)
        function [output, exitflag, info] = forcesInitOutputsC()
            ...
            outputs_u0 = coder.nullcopy(double(zeros(1, 1)));
            output = struct('u0', outputs_u0);
